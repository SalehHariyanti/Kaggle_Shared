# from https://raw.githubusercontent.com/killthekitten/kaggle-ds-bowl-2018-baseline/master/rebuild_mosaics.py
import numpy as np
import pandas as pd
import cv2
import os
...
from settings import base_dir, train_dir, test_dir, train_group_id_file, test_group_id_file, train_mosaics_dir, test_mosaics_dir 

TRAIN_DIR = train_dir
TEST_DIR = test_dir
TRAIN_MOSAICS_DIR = train_mosaics_dir 
TEST_MOSAICS_DIR = test_mosaics_dir
...
def build_mosaics(train_group_id_file=None, test_group_id_file=None):

    if train_group_id_file is not None:
        ...
        for i, row in tqdm(mosaics.iterrows()):
            mosaic_filename = os.path.join(TRAIN_MOSAICS_DIR, str(row["mosaic_idx"]) + ".png")
            mask_mosaic_filename = os.path.join(TRAIN_MOSAICS_DIR, str(row["mosaic_idx"]))
            if not os.path.exists(mosaic_filename) or not os.path.exists(mask_mosaic_filename):
                ...
                np.savez_compressed(mask_mosaic_filename, mask_mosaic=mask_mosaic)

        print("Copying non-mosaic train images to the same place:")
        for i, row in tqdm(non_mosaics.iterrows()):
            non_mosaic_filename = os.path.join(TRAIN_MOSAICS_DIR, str(row["mosaic_idx"]) + ".png")
            non_mosaic_mask_filename = os.path.join(TRAIN_MOSAICS_DIR, str(row["mosaic_idx"]))
            if not os.path.exists(non_mosaic_filename) or not os.path.exists(non_mosaic_mask_filename):
                non_mosaic = cv2.imread(os.path.join(TRAIN_DIR, row["img_id"], "images", row["img_id"] + ".png"))
                cv2.imwrite(non_mosaic_filename, non_mosaic)
                non_mosaic_mask = combine_non_mosaic_masks(row["img_id"], TRAIN_DIR, non_mosaic.shape[:-1])
                np.savez_compressed(non_mosaic_mask_filename, mask_mosaic=non_mosaic_mask)

    if test_group_id_file is not None:
        ...
        for i, row in tqdm(test_mosaics.iterrows()):
            mosaic_filename = os.path.join(TEST_MOSAICS_DIR, str(row["mosaic_idx"]) + ".png")
        ...
        for i, row in tqdm(test_non_mosaics.iterrows()):
            non_mosaic_filename = os.path.join(TEST_MOSAICS_DIR, str(row["mosaic_idx"]) + ".png")
            if not os.path.exists(non_mosaic_filename):
                non_mosaic = cv2.imread(os.path.join(TEST_DIR, row["img_id"], "images", row["img_id"] + ".png"))
                cv2.imwrite(non_mosaic_filename, non_mosaic)

if __name__ == "__main__":
    build_mosaics(train_group_id_file, test_group_id_file)
