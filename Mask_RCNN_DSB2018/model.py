...
def load_image_gt_augment(dataset, config, image_id, augment=False,
    ...
    if augment:
        ...
        # Remove any blank masks after random transform
        valid = np.sum(mask, axis = (0, 1)) > 0
        mask = mask[:, :, valid]
    ...
    # BBoxes may have become invalidated through resizing or augmenting
    # Check for valid masks and remove any invalid
    valid = check_valid_bbox(bbox)
    mask = mask[:, :, valid]
    bbox = bbox[valid]
    class_ids = class_ids[valid]
...
def load_image_gt_augment_nsb(dataset, config, image_id, augment=False,
                  use_mini_mask=False, include_semantic=False):
    # Load image and mask
    image = dataset.load_image(image_id)
    mask, class_ids = dataset.load_mask(image_id)
...
def load_image_gt_augment_scaled(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    # Calculate the scaling you need to use for resize_image_scaled
    # Just hard-coding 1300 here as an average pixel area for each mask...
    # If it works out come back and do this more effectively...
    _, window, scale, padding = utils.resize_image(
        image.copy(),
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
...
def load_image_gt_augment_fixedaspect(dataset, config, image_id, augment=False,
    ...
    if augment:
        ...
        if np.random.random() < config.augmentation_crop:
            ...
            shape_limit = min(image.shape[:2])

            if np.all(np.array(image.shape[:2]) > 256):
                rand_proportion = np.random.uniform(256 / shape_limit, min(1, 512 / shape_limit))                
            else:
                rand_proportion = np.random.uniform(220 / shape_limit, min(1, 256 / shape_limit))

            image, mask = random_crop(image, mask,(int(image.shape[0] * rand_proportion), int(image.shape[1] * rand_proportion)), 2)
...
def load_image_gt_augment_2(dataset, config, image_id, augment=False,
    ...
    if augment:
        # Random crop
        if np.all(np.array(image.shape[:2]) > 512):
            image, mask = random_crop(image, mask, (512, 512), 2)
        # Random transform
        aug = XY_ImageDataGenerator(**config.augmentation_dict)
        image, mask = aug.random_transform(image, mask)
        # Remove any blank masks after random transform
...
def check_valid_bbox(bbox, min_pixels = 4):
    valid = np.ones((len(bbox), ), dtype = np.bool)
    for i in range(len(bbox)):
        y1, x1, y2, x2 = bbox[i][:4]
        if (y2 == y1) or (x2 == x1) or ((y2 - y1) * (x2 - x1)) <= min_pixels:
            valid[i] = False
    return valid
...
class MaskRCNN():
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        try:
            if by_name:
                self.load_weights_from_hdf5_group_by_name(f, layers)
            else:
                topology.load_weights_from_hdf5_group(f, layers)
        except:
            print('LOAD OF WEIGHTS FAILED')
    ...
    def load_weights_from_hdf5_group_by_name(self, f, layers):
        """Implements name-based weight loading.

        (instead of topological weight loading).

        **Layers with weights with non-matching sizes are skipped.**
            ...
            for layer in index.get(name, []):
                ...
                if len(weight_values) != len(symbolic_weights):
                    ...
                    raise ValueError('Layer #' + str(k) +
                                     ...
                                     ' element(s).')
                    """
                    print(' '.join((layer.name, 'weights incompatible')))
                else:
                    # Set values.
...
def get_callbacks(checkpoint_path, learning_rate):
...
def step_lr_schedule(learning_rate):
    ...
    def schedule(epoch):
        lr = base_rate * (1 - reduction)**np.floor(epoch / step)
        print(':'.join(('optimizer learning rate', str(lr))))
        return lr

    return keras.callbacks.LearningRateScheduler(schedule)

def random_crop(x, y, random_crop_size, channel_axis):
    
    channels_first = channel_axis == 0

    if channels_first:
        x = np.moveaxis(x, channel_axis, -1)
        y = np.moveaxis(y, channel_axis, -1) 
    ...
    if (w, h) != random_crop_size:
        ...
        x = x[offsetw:offsetw+random_crop_size[0], offseth:offseth+random_crop_size[1]]

        if y is not None:
            y = y[offsetw:offsetw+random_crop_size[0], offseth:offseth+random_crop_size[1]]
...
class XY_ImageDataGenerator(object):
    '''
    Extension of Keras' ImageDataGenerator to apply transforms to both X and Y masks.
    We cater for cases where Y is a list of outputs, and where Y is categorical output (shape (x_rows * x_cols, n_classes)).
    '''
    def __init__(self,
                 fn_standardise = None,
    ...
    def standardize(self, x):
        ...
        """
        Adaptation of in-built Keras standardize function to allow for bespoke standardisation
        if fn_standarise is populated.
        """

        if self.fn_standardise:
            return self.fn_standardise(x)
        else:
            # Only applied to X
            if self.rescale:
                x *= self.rescale
    ...
    def random_transform(self, x, y = None):
        ...
        # Store copies of the originals so that if augmentations lead to masks being zero completely, 
        # we revert back to the originals (to avoid nan in loss)
        orig_y = y.copy()
        orig_x = x.copy()
        ...
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                ...
                if y is not None:
                    y = [flip_axis(_y, img_col_index) for _y in y]
        ...
        if self.random_crop_shape:
            if np.random.random() < 0.5:
                x, y = self.random_crop(x, y, self.random_crop_shape, img_channel_index)

        if self.contrast_stretching: 
            if np.random.random() < 0.5: 
                p_low, p_high = np.percentile(x, (np.random.randint(1, self.contrast_stretching), np.random.randint(100 - self.contrast_stretching, 100))) 
                x = exposure.rescale_intensity(x, in_range=(p_low, p_high)) 
 
        if self.adaptive_equalization: 
            if np.random.random() < 0.5: 
                x = exposure.equalize_adapthist(x / 255 if np.max(x) > 1 else x, clip_limit=0.03) 
        ...
        if y is not None and self.y_gaussian_blur is not None:
            if np.random.random() < 0.5:
                sigma = np.random.uniform(self.y_gaussian_blur[0], self.y_gaussian_blur[1])
                y = [ndimage.filters.gaussian_filter(_y, sigma = sigma) for _y in y]

        if y is not None:
            y = self.revert_y_shapes(y, original_y_shape, reshape_required, img_channel_index)
            # If any masks have been completely zeroed, then revert back to original to avoid nans
            if not all([np.max(np.sum(_y, axis = -1)) == 1 if required else np.max(_y) == 1 for _y, required in zip(y, reshape_required)]):
                x = orig_x
                y = orig_y
    ...
    def reshape_y(self, y, x_shape, img_row_index, img_col_index, img_channel_index):
        """
        Convert y inputs to match x shape.
        y may be of the same shape as x, or it may be a categorical output which needs to be reshaped.
        In cases where y has multiple channels, we reduce to a single channel via generate_labels() (for speed in calculating transforms)
        before converting back again 
        """

        if isinstance(y, list):

            original_y_shape = [_y.shape for _y in y]

            # Reshape y inputs to match x
            reshape_required = [_y.ndim < len(x_shape) for _y in y]
            y = [_y.reshape((x_shape[img_row_index], x_shape[img_col_index], _y.shape[-1])) if required else _y for _y, required in zip(y, reshape_required)]

            # Convert to single dimension for speed of calcs
            y = [self.generate_labels(_y, img_channel_index == 0) for _y in y]

            # Ensure channels to last axis
            if img_channel_index == 0:
                y = [np.moveaxis(_y, -1, 0) if required else _y for _y, required in zip(y, reshape_required)]

        else:

            original_y_shape = [y.shape]

            # Reshape y input to match x
            reshape_required = y.ndim < len(x_shape) 
            if reshape_required:
                y = y.reshape((x_shape[img_row_index], x_shape[img_col_index], original_y_shape[0][-1]))

            # Convert to single dimension for speed of calcs
            y = self.generate_labels(y, img_channel_index == 0)

            # Ensure channels to last axis
            if img_channel_index == 0 and reshape_required:
                y = np.moveaxis(y, -1, 0)

            # Send back out as a list for consistency with cases of multiple y inputs
            reshape_required = [reshape_required]
            y = [y]

        return y, original_y_shape, reshape_required

    def revert_y_shapes(self, y, original_y_shape, reshape_required, img_channel_index):

        # First, move channels
        if img_channel_index == 0:
            y = [np.moveaxis(_y, 0, -1) if required else _y for _y, required in zip(y, reshape_required)]

        # Convert from labels -> original mask
        y = [self.labels_to_mask(_y, y_shape[-1], img_channel_index == 0) for _y, y_shape in zip(y, original_y_shape)]

        # Reshape to original shapes
        y = [_y.reshape(_y_shape) if required else _y for _y, _y_shape, required in zip(y, original_y_shape, reshape_required)] 

        return y

    def random_crop(self, x, y, random_crop_size, channel_axis):
    
        channels_first = channel_axis == 0

        if channels_first:
            x = np.moveaxis(x, channel_axis, -1)
            y = [np.moveaxis(_y, channel_axis, -1) for _y in y]
        ...
        if (w, h) != random_crop_size:
            ...
            x = resize_img(x[offsetw:offsetw+random_crop_size[0], offseth:offseth+random_crop_size[1]], w, h)

            if y is not None:
                y = [self.crop_y(_y, offsetw, offseth, w, h, random_crop_size) for _y in y]

        if channels_first:
            x = np.moveaxis(x, -1, channel_axis)
            if y is not None:
                y = [np.moveaxis(_y, -1, channel_axis) for _y in y]
            
        return x, y

    def crop_y(self, y, offsetw, offseth, w, h, random_crop_size):
        """
        Use ndimage.zoom with order = 0 for nearest interpolation: preserve distinct values in y
        """
        y_shape = y.shape
        y = ndimage.zoom(y[offsetw:offsetw+random_crop_size[0], offseth:offseth+random_crop_size[1], 0], 
                         (w / random_crop_size[0], h / random_crop_size[1]), order = 0).reshape(y_shape)
        
        return y

    def generate_labels(self, masks, channels_first):
        """
        Generate labels from multiple masks
        In the case of overlaps the last mask is assigned the winner.
        """
        if channels_first:
            masks = list(masks)
        else:
        ...
        labels = labels.reshape((1,) + labels.shape) if channels_first else labels.reshape(labels.shape + (1,))
...
class XY_NumpyArrayIterator(Iterator):

    """
    NumpyArrayIterator adapted to cater for both X and Y augmentations
    (where Y may also be a list of multiple outputs)
    """

    def __init__(self, X, y = None, image_data_generator = None,
                 batch_size=32, shuffle=False, seed=None,
                 dim_ordering='default',
                 save_to_dir=None, save_prefix='', save_format='jpeg'):
        if y is not None:
            if isinstance(y, list):
                if any([len(X) != len(_y) for _y in y]):
        ...
        super(XY_NumpyArrayIterator, self).__init__(X.shape[0], batch_size, shuffle, seed)
    ...
    def next(self):
        ...
        # Y can be single output or multi-output
        multi_output = isinstance(self.y, (list, dict))
        n_y = len(self.y)
        if self.y is not None:
            if multi_output:
                batch_y = [[] for i in range(n_y)]
            else:
                batch_y = []

        # Accumulate X, Y batch with transforms
        for i, j in enumerate(index_array):
            x = self.X[j]
            if self.y is not None:
                if multi_output:
                    label = [self.y[i][j].astype('float32') for i in range(n_y)]
                else:
                    label = self.y[j].astype('float32')
            else:
                label = None
            x, label = self.image_data_generator.random_transform(x.astype('float32'), label)
            if self.image_data_generator.hsv_augmentation:
                x = self.image_data_generator.hsv_augment(x)
            x = self.image_data_generator.standardize(x)
            batch_x.append(x)
            if label is not None:
                if multi_output:
                    for i in range(n_y):
                        batch_y[i].append(label[i])
                else:
        ...
        if batch_y is not None:
            if multi_output:
                batch_y = [np.array(b) for b in batch_y]
            else:
        ...
        return (batch_x, batch_y) if self.y is not None else batch_x
